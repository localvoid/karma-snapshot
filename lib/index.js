    ...
    if (!lastResult.disconnected) {
      if (data && data.snapshot) {
        ...
        if (snapshotConfig.prune && !lastResult.error && lastResult.skipped === 0) {
          ...
          if (prunedCounter > 0) {
            logger.info(prunedCounter + ' snapshots were pruned');
          }
        }

        if (dirty) {
          fs.writeFileSync(snapshotPath, snapshotConfig.pretty ?
            'window.__snapshot__ = ' + JSON.stringify(snapshotData, undefined, 2) + ';\n' :
            'window.__snapshot__=' + JSON.stringify(snapshotData) + ';');
        }
      } else {
