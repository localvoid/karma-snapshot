(function (window) {
  ...
  var prevSnapshotState = window.__snapshot__;
  var nextSnapshotState = {
    update: prevSnapshotState.update,
    suite: {
      visited: false,
      dirty: false,
      children: {},
  ...
  var setSnapshot = function (path, index, code, lang, dirty) {
    ...
    var suite = nextSnapshotState.suite;
    suite.visited = true;
    if (dirty) {
      suite.dirty = true;
    }
  ...
  window.__snapshot__.get = function (path, index) {
    ...
    var suite = prevSnapshotState.suite;
  ...
  var copyMissingSnapshots = function (prevSuite, nextSuite) {
  ...
  window.__karma__.complete = function (data) {
    ...
    // flag.
    copyMissingSnapshots(prevSnapshotState.suite, nextSnapshotState.suite);

    var arg = { snapshot: nextSnapshotState.suite };
    if (data !== undefined) {
      arg = assign({}, data, arg)
    }
